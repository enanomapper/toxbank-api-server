 ...
 * @param <Q>
 */
public class AlertDBResource extends
		FreemarkerQueryResource<ReadAlert, DBAlert> {
		...
		if (variant.getMediaType().equals(MediaType.TEXT_URI_LIST)) {
			...
			return new RDFJenaConvertor<DBAlert, IQueryRetrieval<DBAlert>>(
					new AlertRDFReporter<IQueryRetrieval<DBAlert>>(
							...
							getDocumentation()), variant.getMediaType(),
					filenamePrefix) {
				@Override
			...
			return new OutputWriterConvertor(new AlertHTMLReporter(
					getRequest(), !singleItem, editable), MediaType.TEXT_HTML);
		else
	...
	protected DBUser getUser(Object userKey) throws ResourceException {
		...
		if (userKey != null) {
			if (userKey.toString().startsWith("U"))
				user = new DBUser(new Integer(Reference.decode(userKey
	...
	protected ReadAlert getAlertQuery(Object key, Object userKey,
			String search_name, Object search_value) throws ResourceException {
		...
		if (key == null) {
			...
			if (key.toString().startsWith("A")) {
				singleItem = true;
				ReadAlert query = new ReadAlert(new Integer(
						Reference.decode(key.toString().substring(1))));
				query.setFieldname(user);
				return query;
			}
		}
	...
	@Override
	protected ReadAlert createQuery(Context context, Request request,
	...
	protected CallableProtectedTask<String> createCallable(Method method,
		...
		try {
			DBUser user = getUser(getRequest().getAttributes().get(
					UserDBResource.resourceKey));
			AlertURIReporter reporter = new AlertURIReporter(getRequest(), "");
			DBConnection dbc = new DBConnection(getApplication().getContext(),
					getConfigFile());
			conn = dbc.getConnection();
			return new CallableAlertCreator(method, item, user, reporter, form,

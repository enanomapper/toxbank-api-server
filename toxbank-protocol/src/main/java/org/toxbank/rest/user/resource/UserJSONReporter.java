...
public class UserJSONReporter<Q extends IQueryRetrieval<DBUser>> extends
		QueryReporter<DBUser, Q, Writer> {
	/**
	 * 
	 */
		...
		try {
			...
			if (user.getOrganisations() != null)
				for (Organisation org : user.getOrganisations()) {
					...
					group.append(String.format(formatGroup,
							groupURIReporter.getURI(org), org.getTitle()));
				}
			...
			getOutput().write(
					String.format(
							format,
							uri,
							(user.getID() > 0) ? String.format("\"U%s\"",
									user.getID()) : null,

...
public class ProtocolFactory {
	...
	public static DBProtocol getProtocol(DBProtocol protocol,
			...
			AccessRights accessRights) throws ResourceException {
		...
		for (final Iterator<FileItem> it = items.iterator(); it.hasNext();) {
			...
			try {
				...
				case filename: {
					if (fi.isFormField()) {
						protocol.setDocument(new Document(new URL(fi
								.getString(utf8))));
					} else {
						String originalName = "";
						if (fi.getSize() == 0)
						...
						if (fi.getName() == null)
							...
							String ext = extIndex > 0 ? fi.getName().substring(
									extIndex) : "";
				...
				case template: {
					if (fi.isFormField()) {
						protocol.setDataTemplate(new Template(new URL(fi
								.getString(utf8))));
					} else {
						if (fi.getSize() == 0)
							throw new ResourceException(new Status(
						...
						protocol.setDataTemplate(new Template(file.toURI()
								.toURL()));
					}
					break;
				}
				case project_uri: {
					String s = fi.getString(utf8);
					if ((s != null) && !"".equals(s)) {
						// TODO what do we do if there are projects already ?
						// e.g. in update ?

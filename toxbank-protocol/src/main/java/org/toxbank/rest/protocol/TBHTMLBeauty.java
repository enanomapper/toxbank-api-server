...
public class TBHTMLBeauty extends HTMLBeauty {
	...
	public void writeTopLinks(Writer w, String title, Request request,
			String meta, ResourceDoc doc, Reference baseReference)
			throws IOException {
		w.write(String.format("<a href='%s%s'>Protocols</a>&nbsp;",
				baseReference, Resources.protocol));
		w.write(String.format("<a href='%s%s'>Organisations</a>&nbsp;",
				baseReference, Resources.organisation));
		w.write(String.format("<a href='%s%s'>Projects</a>&nbsp;",
				baseReference, Resources.project));
		w.write(String.format("<a href='%s%s'>Users</a>&nbsp;", baseReference,
				Resources.user));
		w.write(String.format("<a href='%s%s'>Jobs</a>&nbsp;", baseReference,
				TaskResource.resource));
		w.write(String.format(
				"<a href='%s%s' title='Notifications'>Alerts</a>&nbsp;",
				baseReference, NotificationResource.resourceKey));
	}
	...
	@Override
	public void writeTopHeader(Writer w, String title, Request request,
		...
		w.write(String
				.format("<head> <meta property=\"dc:creator\" content=\"%s\"/> <meta property=\"dc:title\" content=\"%s\"/>",
		...
		w.write(String
				.format("<link rel=\"meta\" type=\"application/rdf+xml\" title=\"%s\" href=\"%s\"/>",
						title, ref));

		w.write(String
				.format("<link rel=\"primarytopic\" type=\"application/rdf+xml\" href=\"%s\"/>",
						ref));
		...
		w.write(String
				.format("<span style='float:right;' class='ui-button'><a href='%s/%s'>%s</a></span>\n",
						baseReference.toString(), getLoginLink(), request
								.getClientInfo().getUser() == null ? "Login"
								: "My account"));
	...
	public void writeSearchForm(Writer w, String title, Request request,
		...
		w.write(String.format(
				"<input name='%s' size='40' value='%s' required/>\n",
				AbstractResource.search_param, query_smiles == null ? ""
						: query_smiles));
	...
	public void writeHTMLHeader(Writer w, String title, Request request,
		 ...
		 * ); writeSearchForm(w, title, request, meta); w.write("</div>\n");
		 */
		w.write("\n<div class=\"ui-layout-center\" style=\"clear:both;padding: 5 5 5 5;\">\n");
	}

	@Override
	public void writeHTMLFooter(Writer output, String title, Request request)
			throws IOException {
		Reference baseReference = request == null ? null : request.getRootRef();
		output.write("</div>\n");
